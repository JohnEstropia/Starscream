...
public class WebSocket: NSObject, StreamDelegate {
    ...
    private func initStreamsWithData(_ data: Data, _ port: Int) {
        ...
        if let cipherSuites = self.enabledSSLCipherSuites {
            if let sslContextIn = CFReadStreamCopyProperty(inputStream, CFStreamPropertyKey(kCFStreamPropertySSLContext)) as! SSLContext?,
                sslContextOut = CFWriteStreamCopyProperty(outputStream, CFStreamPropertyKey(kCFStreamPropertySSLContext)) as! SSLContext? {
    ...
    private func dequeueInput() {
        while !inputQueue.isEmpty {
            ...
            work.withUnsafeBytes({ (buffer: UnsafePointer<UInt8>) -> Void in
